  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 8 }}>
      <svg
        ref={svgRef}
        viewBox="0 0 600 500"
        style={{ maxWidth: '100%', maxHeight: '100%', width: 'auto', height: 'auto', overflow: 'visible', cursor: dragging ? 'grabbing' : 'default', userSelect: 'none' }}
      ...
      >
        ...
        {Array.from({ length: connected }).map((_, i) => {
          ...
          return <line key={i} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="#a78bfa" strokeWidth={9} strokeLinecap="round" />;
        })}
        ...
        {dragging && (
          <line
            ...
            stroke="#c084fc" strokeWidth={7} strokeLinecap="round"
            strokeDasharray="10 5" opacity={0.7}
          />
        )}
        ...
        {STAR_POINTS.map(p => {
          ...
          return (
            <g key={p.id} onMouseDown={e => onMouseDown(p.id, e)} style={{ cursor: isFrom ? 'grab' : 'default' }}>
              {/* 클릭 영역 확장 */}
              <circle cx={p.x} cy={p.y} r={34} fill="transparent" />
              {/* 목표 점 강조 링 */}
              {isTo && !dragging && (
                <circle cx={p.x} cy={p.y} r={28} fill="none" stroke="#fde68a" strokeWidth={4} opacity={0.8} />
              )}
              <circle
                cx={p.x} cy={p.y} r={22}
...
const Round2: FC<{ onDone: () => void }> = ({ onDone }) => {
  ...
  const triggerShake = (msg: string) => {
    setShake(true);
    setHint(msg);
    setTimeout(() => { setShake(false); setHint(''); }, 600);
  };

  const advance = () => {
    const next = count + 1;
    if (next >= R2_TOTAL) { onDone(); return; }
    setCount(next);
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden' }}>
      {/* 범례 */}
      <div style={{ position: 'absolute', top: 12, left: 0, right: 0, display: 'flex', justifyContent: 'center', gap: 20, zIndex: 2 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.85rem', fontWeight: 800, color: '#6d28d9' }}>
          <div style={{ width: 18, height: 18, borderRadius: '50%', background: 'linear-gradient(135deg,#a78bfa,#c084fc)' }} />
          한 번 클릭
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.85rem', fontWeight: 800, color: '#db2777' }}>
          <svg width={18} height={18} viewBox="-1 -1 2 2"><path d={starPath(0, 0, 1)} fill="url(#r2g)" /></svg>
          두 번 클릭
        </div>
      </div>
      ...
      {/* 힌트 메시지 */}
      {hint && (
        <div style={{ position: 'absolute', top: 44, left: '50%', transform: 'translateX(-50%)', background: '#f87171', color: 'white', padding: '5px 16px', borderRadius: 20, fontWeight: 700, fontSize: '0.9rem', zIndex: 3, whiteSpace: 'nowrap' }}>
          {hint}
        </div>
      )}
      ...
      >
        {isCircle ? (
          ...
          <div style={{
            ...
            <div style={{ width: '35%', height: '35%', background: 'rgba(255,255,255,0.6)', borderRadius: '50%' }} />
          </div>
        ) : (
          // 더블클릭 타겟: 별
          <svg width={size} height={size} viewBox="-1 -1 2 2" style={{ overflow: 'visible', filter: 'drop-shadow(0 5px 0 #be185d) drop-shadow(0 4px 10px rgba(219,39,119,0.4))' }}>
            <defs>
            ...
            {/* 더블클릭 힌트 표시 */}
            <text y={0.08} textAnchor="middle" fill="white" style={{ fontSize: '0.55px', fontWeight: 900 }}>×2</text>
          </svg>
        )}
      </div>

      {/* 진행 */}
      <div style={{ position: 'absolute', bottom: 14, left: 0, right: 0, textAlign: 'center', fontSize: '1rem', fontWeight: 800, color: '#6d28d9' }}>
        {count} / {R2_TOTAL}
      </div>
...
const Round3: FC<{ onDone: () => void }> = ({ onDone }) => {
  ...
  if (!started) {
    ...
    return (
      <div style={{ textAlign: 'center', padding: 16 }}>
        {failed && <p style={{ color: '#f87171', fontWeight: 800, marginBottom: 8 }}>아쉬워요! 다시 도전해봐요 💪</p>}
        <p style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: 6, color: '#4c1d95' }}>
          구멍에서 올라온 당근을 🥕
        </p>
        <p style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: 16, color: '#4c1d95' }}>
          바구니로 드래그해서 담아요! 🧺
        </p>
        <button
          onClick={() => { setScore(0); setTimeLeft(R3_TIME); setStarted(true); }}
          style={{ padding: '12px 36px', fontSize: '1.1rem', fontWeight: 900, background: 'linear-gradient(135deg,#a78bfa,#c084fc)', color: 'white', border: 'none', borderRadius: 50, cursor: 'pointer', boxShadow: '0 4px 0 #7c3aed' }}
        >
          시작!
        </button>
      </div>
  ...
  return (
    <div
      ref={containerRef}
      style={{ width: '100%', height: '100%', position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, userSelect: 'none' }}
    >
      {/* 상태바 */}
      <div style={{ display: 'flex', gap: 36, fontWeight: 800, color: '#6d28d9', fontSize: '1.2rem' }}>
        <span>⏰ {timeLeft}초</span>
        <span>🥕 {score} / {R3_TARGET}</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
        ...
        <div style={{
          ...
          gap: 12, background: '#ede9fe', padding: 16, borderRadius: 26,
          width: 'min(72vw, 360px)', height: 'min(72vw, 360px)',
        }}>
          {Array.from({ length: R3_HOLES }).map((_, i) => (
            <div
              key={i}
              style={{ borderRadius: '50% 50% 18% 18%', background: '#c4b5fd', position: 'relative', overflow: 'hidden', boxShadow: 'inset 0 5px 10px rgba(0,0,0,0.2)', cursor: i === activeHole ? 'grab' : 'default' }}
              onMouseDown={e => startDrag(e, i)}
            >
              {/* 당근 */}
              <div style={{
                position: 'absolute', bottom: i === activeHole && !dragging ? '6%' : '-120%',
        ...
        <div
          ...
          style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            borderRadius: 24,
            background: basketGlow ? 'rgba(250,204,21,0.4)' : 'rgba(255,255,255,0.6)',
            boxShadow: basketGlow ? '0 0 0 5px #fbbf24' : '0 0 0 2px rgba(167,139,250,0.3)',
            transition: 'all 0.2s',
            flexShrink: 0,
          }}
        >
...
const ChallengeStep: FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  const nextRound = () => {
    ...
    setTimeout(() => { setRound(r => r + 1); setTransitioning(false); }, 900);
  };
        ...
        {ROUNDS.map((_r, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <div style={{
              ...
              background: i < round ? '#10b981' : i === round ? 'linear-gradient(135deg,#a78bfa,#c084fc)' : '#e9d5ff',
              fontSize: 13, fontWeight: 900, color: i <= round ? 'white' : '#a78bfa',
              boxShadow: i === round ? '0 2px 8px rgba(124,58,237,0.3)' : 'none',
              transition: 'all 0.3s',
            }}>
              {i < round ? '✓' : i + 1}
            </div>
            {i < ROUNDS.length - 1 && <div style={{ width: 20, height: 2, background: i < round ? '#10b981' : '#e9d5ff', borderRadius: 2, transition: 'background 0.3s' }} />}
          </div>
        ))}
        <span style={{ marginLeft: 8, fontWeight: 800, color: '#6d28d9', fontSize: '0.9rem' }}>
          {ROUNDS[round].emoji} {ROUNDS[round].label}
        </span>
      </div>

      {/* 라운드 콘텐츠 */}
      <div style={{ flex: 1, minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: transitioning ? 0 : 1, transition: 'opacity 0.4s', position: 'relative' }}>
