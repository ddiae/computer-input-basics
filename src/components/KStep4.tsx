...
export default function KStep4({ onComplete }: Props) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      const specialKeys = ['Enter', ' ', 'Control', 'Shift', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'];
  ...
  return (
    <div style={{
      ...
      {/* 진행도 */}
      <p style={{
        fontSize: 'clamp(0.9rem, 2vw, 1.1rem)', fontWeight: 700,
        color: '#6d28d9', margin: 0,
        background: '#f5f3ff', padding: '6px 20px',
        borderRadius: '20px', border: '2px solid #c084fc',
      }}>
      ...
      <div style={{
        ...
        minWidth: isSpace ? 'clamp(180px, 30vw, 260px)' : 'clamp(120px, 18vw, 180px)',
        ...
        animation: status === 'wrong' ? 'kshake 0.4s ease' : status === 'correct' ? 'kpop 0.35s ease' : 'none',
        ...
        <span style={{
          ...
          {status === 'correct' ? '✅' : status === 'wrong' ? '❌' : current.label}
        </span>
      </div>
      ...
      <p style={{
        ...
        {status === 'wrong' ? '다시 눌러봐요! 👀' : '이 키를 눌러보세요!'}
      </p>
