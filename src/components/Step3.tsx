...
const ANIMALS_DATA = [
  ...
  { id: "ladybug", name: "무당벌레", icon: "🐞", targetId: "grass" },
...
const Step3: FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (houseEl) {
      ...
      const targetX =
        ((hRect.left - cRect.left + hRect.width / 2) / cRect.width) * 100;
      const targetY =
        ((hRect.top - cRect.top + hRect.height / 2) / cRect.height) * 100;
          ...
          const updated = prev.map((a) =>
            a.id === draggingId
              ? {
                  ...a,
                  isHappy: true,
                  currentPos: { x: targetX - 5, y: targetY - 5 }
                }
              : a
          );
          ...
          setTimeout(() => {
            setAnimals((current) => {
              const disappeared = current.map((a) =>
                a.id === draggingId
                  ? { ...a, isPlaced: true, isHappy: false }
                  : a
              );
              if (disappeared.every((u) => u.isPlaced))
    ...
    >
      ...
      >
        {HOUSES.map((house) => (
          ...
          >
            <span style={{ fontSize: "min(12vw, 5rem)", marginBottom: "10px" }}>
              {house.icon}
            </span>
          </div>
        ))}
